...
ip = "10.0.0.34" #change to my new IP
port = 9999 #change to my new PORT
client = SimpleUDPClient(ip, port)  # Create client

args = []
pattern = 118
duration = 0.5 #0.5 # 0.1, 0.5, 1, 1.5
#modifier = 0 # can include additional variables if desired - update server code accordingly to handle

belt = 0 #belt number used here to identify associated RaspPi
#buzz = 0
...
for i in range(len(args)):
		...
		print("Belt_0 Buzzing on #{0}".format(args[i]))
		client.send_message("/belt_0/buzz_{0}".format(args[i]), [duration, pattern]) # sending OSC commands to server/belt
		#client.send_message("/belt_{0}/buzz_{0}".format(belt, buzz, [duration, pattern, modifier])
		print("/belt_0/buzz_{0}".format(args[i]), [duration, pattern])
