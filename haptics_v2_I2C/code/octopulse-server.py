...
import adafruit_drv2605_LOG # toggle this import with next import depending on which file is desired for access
                             # BUT NOT BOTH make sure one or the other is located in ~/.local/lib/python3.7/site-packages/
#import adafruit_drv2605 # this library is a circuit-python dev to directly access hardware of DRV
                        ...
                        # LOCATION: ~/.local/lib/python3.7/site-packages/adafruit_drv2605.py
...
# but you can start a second session to test/see comments printed to screen fir loggin or debugging a custom setup
...
# additional guidance is provided for customising code and hardware access / active patterns etc.
#
######
#
#####
...
# IP  : XX.XX.XX.XX | XX.XX.XX.XX | XX.XX.XX.XX | XX.XX.XX.XX | XX.XX.XX.XX
#
#####
#####
...
class I2CThread(threading.Thread):
    def __init__(self, group=None, target=None, name=None,
        ...
        for i in range(8):
            ...
            self.drv.append(adafruit_drv2605_LOG.DRV2605(self.tca[i]))
            #self.drv[i]._write_u8(_DRV2605_REG_MODE, 0x00)  # out of standby
            #self.drv[i].use_ERM() #adafruit-DRV
            self.drv[i].use_LRA()
            # can call auto-cal from here but currently it is included in the custom sample startup setup
            ...
            #self.drv[i].mode = 0 # 0? #MODE_INTTRIG default
            #self.drv[i].library = 2 #6 #LIBRARY_LRA default is LIBRARY_TS2200A / ERM       
        return
        
    def stop(self, *args, **kwargs):
        # belt number used to identify associated RaspPi
        args[2].put({"start": 0, "belt": 0, "buzz": args[0], "pattern": 0, "duration": 0})
    ...
    def run(self):
        while self._running:
            if not q.empty():
                ...
                if item['start'] == 1:
                    if (b < 8):
                        # the DRV chips permit sequences to be loaded - these can be initialized through 
                        # the adafruit_drv2605 file or during the initialization process just prior
                        # documentation in the DRV PDF provides further details on sequence programming
                        self.drv[b].sequence[0] = adafruit_drv2605_LOG.Effect(item['pattern']) #adafruit-DRV
                        ...
                        #self.drv[b].sequence[4] = adafruit_drv2605.Effect(118) #adafruit-DRV
                        ...
                        self.drv[b].sequence[1] = adafruit_drv2605_LOG.Pause(item['duration']) #adafruit-DRV
...
def stop_buzzer(*args, **kwargs):
    global q
    q.put({"start": 0, "belt": 0, "buzz": args[0], "pattern": 0, "duration": 0}) #args contains the pattern and duration
...
def buzz_handler(address, *args):
    ...
    buzz_args = re.split('\/belt_(\d*)\/buzz_(\d*)(\/?)', address) #REG_EX to handle OSC - this can be customized
    ...
    if duration > 1.27:
        duration = 1.27 #this is documented in the DRV PDF
...
dispatcher.map("/belt_0/*", buzz_handler) # belt_0/* number used here only as identifier for associated RaspPi
...
server = osc_server.ThreadingOSCUDPServer(
    ('10.0.0.34', 9999), dispatcher) # change to my new IP, edit port location if required
                                      # if running a second instance for loging and testing change port
                                      # here and then run python3 octopulse-server.py 
                                      # remember to CTRL+C if you want to continue working on the code
                                      # or sudo reboot to reinitialise the startup setup and autorun
